...
BEGIN
  ...
  SET @letras = REPLACE(@letras,'Veintiseis', 'Veintiséis')
...
CREATE OR ALTER   FUNCTION [dbo].[getTextoSinAcentos] ( @Cadena VARCHAR(1000) ) RETURNS VARCHAR(1000)
AS
BEGIN
        DECLARE @texto VARCHAR(1000) = '';
        SET @texto = (
		SELECT STRING_AGG(nuevos, '') FROM (
        SELECT
        -- convertimos a ASCII para detectar entre mayusculas y minusculas
        	CASE
        		WHEN ASCII(caracter) = 225 THEN 'a'
        		WHEN ASCII(caracter) = 193 THEN 'A'

        		WHEN ASCII(caracter) = 233 THEN 'e'
        		WHEN ASCII(caracter) = 201 THEN 'E'

        		WHEN ASCII(caracter) = 237 THEN 'i'
        		WHEN ASCII(caracter) = 205 THEN 'I'

        		WHEN ASCII(caracter) = 243 THEN 'o'
        		WHEN ASCII(caracter) = 211 THEN 'O'

        		WHEN ASCII(caracter) = 250 THEN 'u'
        		WHEN ASCII(caracter) = 218 THEN 'U'
        		ELSE caracter END AS nuevos
        FROM (
        select substring(a.b, v.number+1, 1) AS caracter
        from (select @Cadena b) a
      ...
      RETURN cast(@texto as VARCHAR(1000))
END
GO