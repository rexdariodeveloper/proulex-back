...
/* ****************************************************************
 * fn_getReporteCriteriosProfesor
 * ****************************************************************
 * Descripción:
 * Autor: -------
 * Fecha: -----
 * Modificado por: Rene Carrillo
 * Modificado fecha: 2022-06-21
 * Versión: 1.0.1
 *****************************************************************
*/

ALTER FUNCTION [dbo].[fn_getReporteCriteriosProfesor](@empleadosId VARCHAR(MAX), @fechaInicio Date, @fechaFin Date)RETURNS @tbl TABLE(
empleadoId int,
...
fechaSuplencia varchar(MAX)
	...
	WHERE EMP_EmpleadoId IN (SELECT * FROM dbo.SplitInts(@empleadosId,',')) AND PROGRU_FechaInicio >= @fechaInicio AND PROGRU_FechaFin <= @fechaFin AND ProgramasGrupos.PROGRU_CMM_EstatusId = 2000620
