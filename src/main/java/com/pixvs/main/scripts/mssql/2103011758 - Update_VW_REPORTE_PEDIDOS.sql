...
SELECT
  ...
  ( CASE WHEN ( coalesce(PEDD_CantidadSurtida,0.00) - coalesce(recibo.cantidad, 0.00) ) <> 0 THEN ( coalesce(PEDD_CantidadSurtida,0.00) - coalesce(recibo.cantidad, 0.00) ) ELSE NULL END) AS transito,
