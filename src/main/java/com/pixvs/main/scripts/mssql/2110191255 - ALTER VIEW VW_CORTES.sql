/**
* Created by Angel Daniel Hernández Silva on 18/10/2021.
* Object:  ALTER VIEW [dbo].[VW_CORTES]
*/
...
(
	SELECT
		...
		referenciaPago,
		FORMAT(fechaPago,'dd/MM/yyyy HH:mm:ss') AS fechaPago,
	...
	INNER JOIN (
		...
		FROM (
			SELECT
				id,
				codigo,
				COALESCE(fechaModificacion,fechaCreacion) AS fechaPago,
				estatusId,
				sucursalCorteCajaId,
				medioPagoPVId,
				referenciaPago,
		...
		GROUP BY id, codigo, fechaPago, estatusId, sucursalCorteCajaId, medioPagoPVId, referenciaPago, montoTotal, detallePadreId, detallePadreArticuloId, detallePadreMontoCurso, detallePadreMontoOtros
