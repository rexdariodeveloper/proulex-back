CREATE OR ALTER VIEW [dbo].[VW_LISTADO_SOLICITUDES_PAGO_RH] AS
	...
	WHEN 2000358 THEN 
	(	Select 
		SUM(CASE WHEN INCD.CPXSPRHID_CMM_TipoId is not null THEN (INCD.CPXSPRHID_SalarioDiario * INCD.CPXSPRHID_Dias) ELSE 0 END) - SUM(CASE WHEN INCD.CPXSPRHID_CMM_TipoMovimientoId is not null THEN INCD.CPXSPRHID_SalarioDiario ELSE 0 END)
