...
AS
    ...
    SELECT @stmt = '
        SELECT
            ...
            Emp.EMP_CodigoEmpleado AS "codigo Empleado",
			...
			(Select Cmm.CMM_Valor from ControlesMaestrosMultiples Cmm where Cmm.CMM_ControlId = Emp.EMP_CMM_GradoEstudiosId) as gradoEstudios,
			(Select Cmm.CMM_Valor from ControlesMaestrosMultiples Cmm where Cmm.CMM_ControlId = Emp.EMP_CMM_NacionalidadId) as nacionalidad,
			...
			(Select STRING_AGG(CONCAT(EMPBE_Nombre,'' '',EMPBE_PrimerApellido,'' '',EMPBE_SegundoApellido), '', '') from EmpleadosBeneficiarios WHERE EMPBE_EMP_EmpleadoId=Emp.EMP_EmpleadoId GROUP BY EMPBE_EMP_EmpleadoId) as beneficiarios,
			(CASE WHEN EMP_SalarioDiario = 0 OR EMP_SalarioDiario IS NULL THEN '''' ELSE CAST(EMP_SalarioDiario AS VARCHAR) END) AS "salario Diario",
            (CASE WHEN moneda.MON_Nombre IS NULL OR EMP_SalarioDiario = 0 OR EMP_SalarioDiario IS NULL THEN '''' ELSE CONCAT(moneda.MON_Nombre,''('', MON_Codigo ,'')'') END) AS moneda,
            '+@cols+'
        FROM
            Empleados Emp
            ...
            LEFT JOIN Monedas moneda ON moneda.MON_MonedaId = Emp.EMP_MON_MonedaId
            LEFT JOIN
                (
                    SELECT
