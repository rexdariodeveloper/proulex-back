...
ALTER VIEW [dbo].[VW_REPORTE_EXCEL_ALUMNOS] AS

    SELECT
        ALU_Codigo AS "Código alumno",
        ALU_Nombre AS "Nombre",
        ALU_PrimerApellido + COALESCE(' ' + ALU_SegundoApellido,'') AS "Apellidos",
		CONCAT_WS(' ',CAST(DATEDIFF(MONTH,ALU_FechaNacimiento,GETDATE()) / 12 AS nvarchar(MAX)),'años,',CAST(DATEDIFF(MONTH,ALU_FechaNacimiento,GETDATE()) % 12 AS nvarchar(MAX)),' meses') AS "Edad",
