...
ALTER VIEW [dbo].[VW_GRUPOS_HORARIOS]
WITH SCHEMABINDING
AS
(
	select grupoId, grupo, modalidadId, horarioId, modalidad, dia, horaInicio, horaFin, horas from
	(
		select 
		...
		from 
			[dbo].[PAModalidades] 
			INNER JOIN [dbo].[PAModalidadesHorarios] on PAMOD_ModalidadId = PAMODH_PAMOD_ModalidadId
		...
		where Activo = 1
		) t INNER JOIN [dbo].[ProgramasGrupos] on PROGRU_PAMODH_PAModalidadHorarioId = horarioId
	) modalidadesFijas
	UNION ALL
	(
	select 
	...
	from [dbo].[ProgramasGruposIncompanyHorarios]
	INNER JOIN [dbo].[ProgramasGrupos] on PGINCH_PROGRU_GrupoId = PROGRU_GrupoId
