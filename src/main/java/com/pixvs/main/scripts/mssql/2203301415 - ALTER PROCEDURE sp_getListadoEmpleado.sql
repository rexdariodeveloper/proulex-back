...
GO
ALTER   PROCEDURE [dbo].[sp_getListadoEmpleados]
AS
    DECLARE
        @cols  nvarchar(max),
        @stmt  nvarchar(max)
    ...
    SELECT @stmt = '
        SELECT
			...
			(Select Cmm.CMM_Valor from ControlesMaestrosMultiples Cmm where Cmm.CMM_ControlId = Emp.EMP_CMM_EstatusId) as Estatus,
        ...
        WHERE
            EMP_CMM_EstatusId NOT IN (2000951)'
