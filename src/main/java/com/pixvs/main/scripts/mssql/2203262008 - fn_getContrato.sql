...
(
    SELECT
        ...
        (CASE WHEN PROGRU_CMM_TipoGrupoId = 2000390 OR SUC_PlantelesBandera = 0
            THEN
            ...
            ELSE
            CONCAT(SUC_Domicilio, ' Colonia ', SUC_Colonia, ', ', estSuc.EST_Nombre, ', ', paisSuc.PAI_Nombre, ' - ', SP_Direccion,(CASE WHEN SP_Colonia IS NULL THEN '' ELSE ', Colonia ' END),SP_Colonia,' ',SP_Municipio,(CASE WHEN SP_Direccion IS NULL THEN '' ELSE ',' END),' Jalisco, México')
            END
        ) AS domicilioPlantel,
