...
AS
	...
	FROM CXCFacturas f
		...
		INNER JOIN OrdenesVenta ov ON f.CXCF_FacturaId = ov.OV_CXCF_FacturaId
	...
	GROUP BY f.CXCF_FacturaId, f.CXCF_Fecha, s.SUC_SucursalId, s.SUC_Nombre, tf.CMM_Valor, ISNULL(f.CXCF_Serie + ' ', '') + f.CXCF_Folio, f.CXCF_ReceptorRFC, f.CXCF_ReceptorNombre, fd.CXCFD_Importe, e.CMM_ControlId, e.CMM_Valor, ov.OV_Codigo
GO