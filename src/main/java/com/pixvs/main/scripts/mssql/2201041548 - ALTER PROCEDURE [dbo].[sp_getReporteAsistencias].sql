...
GO
CREATE OR ALTER PROCEDURE [dbo].[sp_getReporteAsistencias] @usuarioId INT, @grupoId INT
AS
	...
	SELECT @cols = ISNULL(@cols + ', ', '') + '[' + FORMAT(T.fecha,'yyyy-MM-dd') + ']' FROM (SELECT DISTINCT CAST(AA_Fecha AS DATE) fecha FROM AlumnosAsistencias) AS T
