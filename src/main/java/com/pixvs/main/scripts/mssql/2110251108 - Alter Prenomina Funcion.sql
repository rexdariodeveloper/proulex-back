...
AS BEGIN
	...
	FROM(
		...
		FROM(
			...
			left join TabuladoresCursos on TABC_PROGI_ProgramaIdiomaId = PROGI_ProgramaIdiomaId AND TABC_PAMODH_PAModalidadHorarioId = PROGRU_PAMODH_PAModalidadHorarioId
		...
		FROM(
				select
					...
					PROGRU_SueldoProfesor as sueldoPorHora
				from Empleados
				INNER join ProgramasGruposListadoClases ON PROGRULC_CMM_FormaPagoId IN (2000520,2000522) AND PROGRULC_EMP_EmpleadoId = EMP_EmpleadoId AND PROGRULC_FechaCreacion <= @fechaFinQuincena
		...
		left join TabuladoresCursos on TABC_PROGI_ProgramaIdiomaId = PROGI_ProgramaIdiomaId AND TABC_PAMODH_PAModalidadHorarioId = PROGRU_PAMODH_PAModalidadHorarioId
		left join Tabuladores on TAB_TabuladorId = TABC_TAB_TabuladorId
		--left join TabuladoresDetalles on TABD_TAB_TabuladorId = TAB_TabuladorId AND TABD_PAPC_ProfesorCategoriaId = EMPCA_PAPC_ProfesorCategoriaId
		...
		FROM(
				...
				from Empleados
				inner join ProgramasGrupos on PROGRU_EMP_EmpleadoId = EMP_EmpleadoId
				INNER join ProgramasGruposListadoClases ON PROGRULC_CMM_FormaPagoId IN (2000520,2000522) AND PROGRULC_PROGRU_GrupoId = PROGRU_GrupoId AND PROGRULC_FechaCreacion <= @fechaFinQuincena and PROGRULC_Fecha < @fechaInicioQuincena
				left join Sucursales on SUC_SucursalId = PROGRU_SUC_SucursalId
				left join PAModalidades on PAMOD_ModalidadId = PROGRU_PAMOD_ModalidadId
				inner join PAModalidadesHorarios ON PAMODH_PAModalidadHorarioId = PROGRU_PAMODH_PAModalidadHorarioId
				left join ProgramasIdiomas on PROGI_ProgramaIdiomaId = PROGRU_PROGI_ProgramaIdiomaId
