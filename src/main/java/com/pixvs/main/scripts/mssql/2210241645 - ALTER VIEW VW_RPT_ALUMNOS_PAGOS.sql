...
-- Create date: ------
-- Modified date: 24/10/2022
-- Description:	Vista para obtener el reporte de Alumnos Pagos
-- ==============================================
...
AS
     SELECT ALU_CodigoAlumnoUDG codigo,
            ...
            --CASE WHEN INS_CMM_EstatusId = 2000510 AND ov.OV_CMM_EstatusId IN (2000507, 2000508) AND OV_CMM_MetodoPago IS NULL THEN ov.OV_Codigo ELSE NULL END poliza, -- Solo mostrar Pagada o Facturada
			CASE WHEN INS_CMM_EstatusId = 2000510 THEN ov.OV_Codigo ELSE NULL END poliza,
			--CASE WHEN INS_CMM_EstatusId = 2000510 AND ov.OV_CMM_EstatusId IN (2000507, 2000508) AND OV_CMM_MetodoPago IS NULL THEN montos.Total ELSE NULL END precio,
			--ov.OV_Codigo poliza, -- Solo mostrar Pagada o Facturada
			montos.Total precio,
     ...
     FROM Inscripciones
          ...
          INNER JOIN OrdenesVenta ov ON OVD_OV_OrdenVentaId = ov.OV_OrdenVentaId
          LEFT JOIN
		  (
          ...
          LEFT JOIN Sucursales Sedes ON ov.OV_SUC_SucursalId = Sedes.SUC_SucursalId
		  ...
		  OUTER APPLY dbo.fn_getDatosFacturacionOV(Sedes.SUC_SucursalId, ov.OV_Codigo) AS montos
