...
-- Description:	La vista de Nota de Venta
-- ==============================================
ALTER VIEW [dbo].[VW_NotaVenta] AS
	...
	SELECT
		...
		autofacturaURL.CMA_Valor AS autofacturaURL,
		CONCAT('CUENTA CON ', CASE WHEN tipoVigenciaFacturar.CMA_Valor = '1000061' /*Fin de mes*/ THEN 'HASTA EL ÚLTIMO DÍA DEL MES' ELSE CONCAT(diasFacturar.CMA_Valor, ' DIAS') END, ' PARA EMITIR SU FACTURA') AS leyendaFacturar
			...
			FROM(
				...
				FROM(
					SELECT
						...
						CASE
							WHEN INS_InscripcionId IS NOT NULL THEN PROGRU_Codigo
							WHEN OVD_OVD_DetallePadreId IS NOT NULL THEN ''
							ELSE ''
					...
					LEFT JOIN Inscripciones ON INS_OVD_OrdenVentaDetalleId = OVD_OrdenVentaDetalleId AND INS_CMM_EstatusId IN (2000510, CASE WHEN COALESCE(MPPV_Nombre, '') = 'Centro de pagos' THEN 2000511 END)
	...
	LEFT JOIN ControlesMaestros AS autofacturaURL ON autofacturaURL.CMA_Nombre = 'CMA_Autofactura_URL'
