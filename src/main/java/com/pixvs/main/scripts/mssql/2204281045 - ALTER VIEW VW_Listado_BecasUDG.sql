/**
* Created by Angel Daniel Hernández Silva on 28/04/2022.
...
CREATE OR ALTER VIEW [dbo].[VW_Listado_BecasUDG] AS
	...
	SELECT
		...
		COALESCE(EntidadBeca.CMM_Valor,'') AS entidadBeca,
	...
	LEFT JOIN Alumnos
		ON (
			...
			OR (
				...
				AND COALESCE(ALU_CodigoUDG,'') = COALESCE(BECU_CodigoEmpleado,'')
	...
	LEFT JOIN ControlesMaestrosMultiples AS EntidadBeca ON EntidadBeca.CMM_ControlId = BECU_CMM_EntidadId
