...
DROP FUNCTION IF EXISTS fn_getKardexArticulo
GO

CREATE FUNCTION [dbo].[fn_getKardexArticulo] ( @articuloId INT,
...
BEGIN
	...
	DECLARE @existencias TABLE (IM_ART_ArticuloId INT, IM_LOC_LocalidadId INT, IM_Cantidad DECIMAL(28, 10), IM_FechaCreacion DATETIME, IM_Referencia NVARCHAR(1), IM_Razon NVARCHAR(1), CMM_Valor NVARCHAR(1), IM_USU_CreadoPorId INT)
	...
	(
		...
		(
			...
			WHERE IM_ART_ArticuloId = ISNULL(@articuloId, IM_ART_ArticuloId)
					AND IM_FechaCreacion BETWEEN @fechaCreacionDesde AND @fechaCreacionHasta
					AND LOC_ALM_AlmacenId = ISNULL(@almacenId, LOC_ALM_AlmacenId)
					AND ISNULL(@localidades, '|' + CAST(IM_LOC_LocalidadId AS NVARCHAR(10)) + '|') LIKE '|' + CAST(IM_LOC_LocalidadId AS NVARCHAR(10)) + '|'
