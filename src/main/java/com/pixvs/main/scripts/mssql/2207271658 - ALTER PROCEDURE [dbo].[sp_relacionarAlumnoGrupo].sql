...
AS
	BEGIN
		...
		BEGIN TRY
			...
			SELECT @alumnoNuevo = CAST(CASE WHEN COUNT(*) > 0 THEN 1 ELSE 0 END AS BIT) FROM Inscripciones WHERE INS_ALU_AlumnoId = 28289 AND INS_CMM_EstatusId <> 2000512;
			...
			IF @alumnoNuevo = 0 AND [dbo].[fn_getNivelAlumnoPermiteInscripcion] (@alumnoId,@idiomaId,@programaId) < @nivel
			BEGIN
			...
			IF [dbo].[fn_getAlumnoInterferenciaHorario](@alumnoId, @horarioId) = 1
			BEGIN
				SET @msg = (SELECT FORMATMESSAGE(N'El alumno %s ya cuenta con una inscripción para el mismo horario', @codigoAlumno));
				THROW 50000, @msg, 1
			END
			/* Si es de plantilla, validar por el programa */
			IF @origenId = 2000981
			BEGIN
				IF [dbo].[fn_getAlumnoCursandoIdioma] (@alumnoId, @idiomaId, @programaId) = 1
				BEGIN
					SET @msg = (SELECT FORMATMESSAGE(N'El alumno %s ya cuenta con una inscripción para el mismo programa', @codigoAlumno));
					THROW 50000, @msg, 1
				END
			END
			/* En caso contrario, validar por el idioma */
			ELSE
			BEGIN
				IF [dbo].[fn_getAlumnoCursandoIdioma] (@alumnoId, @idiomaId, NULL) = 1
				BEGIN
					SET @msg = (SELECT FORMATMESSAGE(N'El alumno %s ya cuenta con una inscripción para el mismo idioma', @codigoAlumno));
					THROW 50000, @msg, 1
				END
			END
