		...
		BEGIN TRY
			BEGIN TRANSACTION
				...
				IF (@version = '4.0' AND (@receptorCP IS NULL OR @receptorCP = ''))
					SET @mensaje += 'No se ha configurado el CP del Receptor.'

				IF (@receptorRFC IS NULL OR @receptorRFC = '')
					SET @mensaje += 'No se ha configurado el RFC del Receptor.'

				IF (@version = '4.0' AND (@receptorNombre IS NULL OR @receptorNombre = ''))
					SET @mensaje += 'No se ha configurado el Nombre del Receptor.'
					
				IF (@version = '4.0' AND @receptorRegimenFiscalId IS NULL)
					SET @mensaje += 'No se ha configurado el Régimen Fiscal del Receptor.'
				...
				IF(@claveProducto IS NULL OR @claveProducto = '')
					SET @mensaje += 'El artículo [' + @articulo + '] no tiene Clave de Producto.'

				IF(@version = '4.0' AND @objetoImpuestoId IS NULL)
					SET @mensaje += 'No se ha configurado el Objeto de Impuesto para el artículo [' + @articulo + '].'

				IF(@claveUM IS NULL OR @claveUM = '')
					SET @mensaje += 'No se ha configurado la clave para la UM [' + @um + '].'
