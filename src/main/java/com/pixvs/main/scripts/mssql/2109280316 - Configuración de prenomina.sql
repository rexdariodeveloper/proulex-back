	...
	FROM(
		...
		FROM(
			...
			select
				...
				0 as deduccion,
				(Select count(*) from dbo.fn_getDiaHabiles(@fechaInicioQuincena,@fechaFinQuincena,PAMOD_Domingo,PAMOD_Lunes,PAMOD_Martes,PAMOD_Miercoles,PAMOD_Jueves,PAMOD_Viernes,PAMOD_Sabado) where fecha is not null)* DATEDIFF(HOUR,PAMODH_HoraInicio,PAMODH_HoraFin) as horasTotales,
		...
		where EMP_FechaAlta <= fecha AND fecha is not null AND concat(PROGRU_GrupoId,fecha) not in (Select concat(PRENO_PROGRU_GrupoId,PRENO_DiasFestivo) from Prenominas where PRENO_DiasFestivo >= @fechaInicioQuincena and PRENO_DiasFestivo <=@fechaFinQuincena)
		)Q5
