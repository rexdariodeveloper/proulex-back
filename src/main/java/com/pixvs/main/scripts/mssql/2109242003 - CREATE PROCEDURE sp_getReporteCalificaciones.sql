...
GO
CREATE OR ALTER PROCEDURE [dbo].[sp_getReporteCalificaciones] @usuarioId INT, @programaIdiomaId INT, @grupoId INT
AS
	DECLARE
		@cols  nvarchar(max),
		@stmt  nvarchar(max),
	...
	SELECT @cols = ISNULL(@cols + ', ', '') + '[' + PAAE_Actividad + ']' from ProgramasIdiomasNiveles
