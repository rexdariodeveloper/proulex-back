...
ALTER FUNCTION [dbo].[fn_getKardexAlumno](@alumnoId int)RETURNS @tbl TABLE(
	...
	SELECT
		...
		CASE WHEN LEN(ISNULL(OV_Codigo, '')) != 36 AND ISNULL(OV_CMM_EstatusId, -1) IN (2000507, 2000508) AND OV_CMM_MetodoPago IS NOT NULL THEN ISNULL(OV_Codigo, '') ELSE '' END AS alumnoOrdenVenta,
		CONVERT(BIT, CASE WHEN ISNULL(OV_CMM_EstatusId, -1) IN (2000507, 2000508) AND OV_CMM_MetodoPago IS NOT NULL THEN 1 ELSE 0 END) AS imprimirOV, -- Solo mostrar Pagada o Facturada
		...
		CASE WHEN (GETDATE() < PROGRU_FechaFin AND PROGRU_CMM_EstatusId != 2000621) OR ALUG_CMM_EstatusId = 2000677 THEN '' ELSE CAST(CAST(COALESCE(ALUG_CalificacionFinal, 0) AS INT) AS VARCHAR(3)) END AS grupoCalificacion
