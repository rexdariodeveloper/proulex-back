/**
 * Created by Angel Daniel Hernández Silva on 26/07/2021.
 * Object:  ALTER VIEW [dbo].[VW_PROGRAMACION_PAGO_PROJECTION_CXPFactura]
 */
...
SELECT
	id,
	MAX(proveedorId) proveedorId,
	MAX(codigoRegistro) codigoRegistro,
	MAX(montoRegistro) montoRegistro,
	MAX(saldo) saldo,
	MAX(montoProgramado) montoProgramado,
	MAX(fechaRegistro) fechaRegistro,
	STRING_AGG(ordenCompraTexto, ', ') ordenCompraTexto,
	MAX(folioSolicitudPagoServicio) folioSolicitudPagoServicio,
	MAX(evidenciaStr) evidenciaStr,
	MAX(evidenciaRh) evidenciaRh,
	MAX(facturaPDFStr) facturaPDFStr,
	MAX(facturaXMLStr) facturaXMLStr,
	MAX(cxpSolicitudesPagosServiciosStr) cxpSolicitudesPagosServiciosStr,
	MAX(diasCredito) diasCredito,
