...
CREATE VIEW [dbo].[VW_Listado_PlantillaGrupos]
AS
	...
	FROM  ProgramasGrupos pg
		...
		LEFT JOIN (SELECT COUNT(*) AS Cupo, i.INS_PROGRU_GrupoId AS GrupoId FROM Inscripciones i GROUP BY i.INS_PROGRU_GrupoId) AS cupo ON pg.PROGRU_GrupoId = cupo.GrupoId
		LEFT JOIN (SELECT COUNT(*) AS Inscrito, i.INS_PROGRU_GrupoId AS GrupoId FROM Inscripciones i WHERE i.INS_CMM_EstatusId = 2000510 GROUP BY i.INS_PROGRU_GrupoId) AS inscrito ON pg.PROGRU_GrupoId = inscrito.GrupoId
