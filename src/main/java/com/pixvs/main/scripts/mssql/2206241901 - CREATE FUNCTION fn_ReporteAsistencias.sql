...
-- Modify date:
-- Description:	Funcion para obtener el reporte de asistencia
-- Version 1.0.0
-- =============================================
CREATE OR ALTER FUNCTION [dbo].[fn_ReporteAsistencias]
(
	@GrupoId INT
)
RETURNS TABLE
...
(
	SELECT ROW_NUMBER() OVER(ORDER BY a.ALU_PrimerApellido ASC) AS Numero,
		...
		COALESCE (ag.ALUG_Faltas, 0) AS Faltas,
		COALESCE (ag.ALUG_Asistencias, 0) AS Asistencias,
		COALESCE (ag.ALUG_MinutosRetardo, 0) AS Retardos,
