...
AS
	...
	FROM CXCFacturas f
		...
		LEFT JOIN (SELECT ov.OV_CXCF_FacturaId,
						CASE WHEN COUNT(ov.OV_CXCF_FacturaId) > 1 THEN 'VARIAS' ELSE (SELECT _ov.OV_Codigo FROM OrdenesVenta _ov WHERE _ov.OV_CXCF_FacturaId = ov.OV_CXCF_FacturaId) END AS NotaVenta
					FROM OrdenesVenta ov
					WHERE NOT ov.OV_CXCF_FacturaId IS NULL
					GROUP BY ov.OV_CXCF_FacturaId) AS ov ON f.CXCF_FacturaId = ov.OV_CXCF_FacturaId
