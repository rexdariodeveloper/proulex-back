		...
		BEGIN TRY
			BEGIN TRANSACTION
				...
				INSERT INTO CXCFacturasDetalles
						...
						Subtotal AS Importe,
						Descuento,
						CASE WHEN objetoImp.OVId IS NOT NULL THEN @ObjetoImpId_002 ELSE @ObjetoImpId_001 END AS ObjetoImpuestoId,
				...
				FROM OrdenesVenta AS ov
						...
						(
								SELECT ov.OV_OrdenVentaId AS OVId,
									   SUM(CASE WHEN ART_CMM_ObjetoImpuestoId = @ObjetoImpId_002 THEN 1 ELSE 0 END) AS ObjetoImp_002
								FROM OrdenesVenta AS ov
									 INNER JOIN @ordenesVenta ON ov.OV_OrdenVentaId = Id
