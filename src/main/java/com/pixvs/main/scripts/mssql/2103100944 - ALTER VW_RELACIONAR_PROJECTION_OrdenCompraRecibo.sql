...
CREATE OR ALTER VIEW [dbo].[VW_RELACIONAR_PROJECTION_OrdenCompraRecibo] AS
	...
	SELECT
		...
		cantidadRecibo,
		totalRecibido - COALESCE(SUM(CASE WHEN COALESCE(CXPF_CMM_EstatusId,2000111) IN (2000111,2000114) THEN NULL ELSE CXPFD_Cantidad END),0) AS cantidadPendienteRelacionar,
	...
	HAVING totalRecibido - COALESCE(SUM(CASE WHEN COALESCE(CXPF_CMM_EstatusId,2000111) IN (2000111,2000114) THEN NULL ELSE CXPFD_Cantidad END),0) > 0
