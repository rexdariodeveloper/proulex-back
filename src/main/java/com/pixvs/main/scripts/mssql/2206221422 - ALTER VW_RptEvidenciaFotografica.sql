...
CREATE OR ALTER VIEW [dbo].[VW_RptEvidenciaFotografica] AS
	SELECT PROGRU_GrupoId AS GrupoId,
		   ...
		   UPPER(CONCAT_WS(' ', EMP_Nombre, EMP_PrimerApellido, EMP_SegundoApellido)) AS Profesor,
		   Nombre1,
		   Ruta1,
		   Nombre2,
		   Ruta2,
		   ...
		   ROW_NUMBER() OVER (ORDER BY PROG_Codigo, SUC_Nombre, CONCAT(PROG_Codigo,' ',CMM_Valor), CONCAT_WS(' - ', CAST(PAMODH_HoraInicio AS NVARCHAR(5)), CAST(PAMODH_HoraFin AS NVARCHAR(5))), UPPER(CONCAT_WS(' ', EMP_Nombre, EMP_PrimerApellido, EMP_SegundoApellido)), Evidencia1, Evidencia2) AS Orden
