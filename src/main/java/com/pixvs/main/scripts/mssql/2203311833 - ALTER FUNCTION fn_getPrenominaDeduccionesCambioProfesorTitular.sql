...
AS BEGIN
	...
	FROM(
		...
		SELECT
			...
			PRENOM_PrenominaMovimientoId AS referenciaProcesoId
		FROM(
			SELECT
			...
			INNER JOIN PrenominaMovimientos AS MovimientoPercepcion
				...
				AND MovimientoPercepcion.PRENOM_ReferenciaProcesoId = PROGRU_GrupoId
			/*INNER JOIN Prenominas
				ON PRENO_EMP_EmpleadoId = EMP_EmpleadoId
				AND PRENO_PROGRU_GrupoId = PROGRU_GrupoId
				AND PRENO_DiasFestivo IS NULL
				AND PRENO_FechaClaseSuplida IS NULL
				AND PRENO_FechaInicioQuincena = @fechaInicioQuincena
				AND PRENO_FechaFinQuincena = @fechaFinQuincena*/
