    ...
    FROM
        ...
        SELECT * FROM
            (
                ...
                WHERE PROGRU_CMM_EstatusId <> 2000621 AND PROGIN_PROGI_ProgramaIdiomaId = ' + CAST(@programaIdiomaId as nvarchar) + ' AND AEC_PROGRU_GrupoId = '+ COALESCE(CAST(@grupoId as nvarchar),'AEC_PROGRU_GrupoId') +'
            ) detalles
        ) t
        PIVOT(
            MAX(puntaje)
