...
BEGIN
	...
	SELECT Id,
		   ...
		   Total,
		   SucursalId,
		   SP_SucursalPlantelId AS PlantelId,
		   EstatusId,
		   OV_USU_CreadoPorId AS UsuarioId,
		   FacturaId
	FROM dbo.OrdenesVenta AS ov
		 CROSS APPLY dbo.fn_getDatosFacturacionOV(ov.OV_SUC_SucursalId, ov.OV_Codigo) AS datosOV
		 INNER JOIN dbo.SucursalesCortesCajas ON OV_SCC_SucursalCorteCajaId = SCC_SucursalCorteCajaId
		 LEFT JOIN dbo.SucursalesPlanteles ON SCC_SP_SucursalPlantelId = SP_SucursalPlantelId
