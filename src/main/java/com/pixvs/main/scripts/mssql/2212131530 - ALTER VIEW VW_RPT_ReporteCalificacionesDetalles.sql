...
AS
	...
	FROM AlumnosGrupos ag
		...
		INNER JOIN Inscripciones i ON ag.ALUG_ALU_AlumnoId = i.INS_ALU_AlumnoId AND ag.ALUG_PROGRU_GrupoId = i.INS_PROGRU_GrupoId AND INS_CMM_EstatusId NOT IN (2000512, 2000513) /* Cancelada*/
		...
		INNER JOIN (SELECT ins.INS_ALU_AlumnoId alumnoId, p.PROGRU_GrupoId grupoId, pa.PACIC_Codigo codigo
		...
		LEFT JOIN (
			...
			FROM (
				...
				SELECT aec.AEC_ALU_AlumnoId, aec.AEC_PROGRU_GrupoId, paae.PAAE_Codigo, aec.AEC_Puntaje
				FROM AlumnosExamenesCalificaciones aec
					INNER JOIN ProgramasGruposExamenesDetalles pged ON aec.AEC_PROGRUED_ProgramaGrupoExamenDetalleId = pged.PROGRUED_ProgramaGrupoExamenDetalleId
					INNER JOIN PAActividadesEvaluacion paae ON pged.PROGRUED_PAAE_ActividadEvaluacionId = paae.PAAE_ActividadEvaluacionId
