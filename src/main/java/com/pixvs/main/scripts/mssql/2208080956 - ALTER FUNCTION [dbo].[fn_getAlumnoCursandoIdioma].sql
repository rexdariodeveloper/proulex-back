...
/**
* Created by Angel Daniel Hernández Silva on 04/08/2022.
*/

ALTER FUNCTION [dbo].[fn_getAlumnoCursandoIdioma] (@alumnoId int, @idiomaId int, @programaId int, @soloInscripcionesPagadas bit)
RETURNS bit
AS
BEGIN
    ...
    IF @programaId IS NULL BEGIN
        ...
        WHERE
            ...
            AND(
                (@soloInscripcionesPagadas = 0 AND INS_CMM_EstatusId IN (2000510,2000511)) -- (Pagadas y pendientes de pago)
                OR (@soloInscripcionesPagadas = 1 AND INS_CMM_EstatusId = 2000510) -- (Solo pagadas)
            )
