/****** Object:  StoredProcedure [dbo].[sp_getReporteCalificaciones]    Script Date: 11/04/2022 06:41:53 p. m. ******/
SET ANSI_NULLS ON
GO
    ...
    FROM
        ...
        SELECT * FROM
            (
                ...
                WHERE PROGIN_PROGI_ProgramaIdiomaId = ' + CAST(@programaIdiomaId as nvarchar) + ' AND AEC_PROGRU_GrupoId = '+ CAST(@grupoId as nvarchar) +'
            ) detalles
        ) t
        PIVOT(
            MAX(puntaje)
