/**
* Created by Angel Daniel Hernández Silva on 06/04/2022.
...
CREATE OR ALTER VIEW [dbo].[VW_HistorialPVResumen_OrdenesVentaDetalles] AS
	SELECT
		...
		CASE
			WHEN COALESCE(INS_InscripcionId,INSSG_InscripcionId,ALUEC_AlumnoExamenCertificacionId) IS NOT NULL THEN CONCAT('Alumno: ',ALU_Nombre,' ' + ALU_PrimerApellido,' ' + ALU_SegundoApellido)
			WHEN OVD_OVD_DetallePadreId IS NOT NULL THEN ''
			ELSE ''
