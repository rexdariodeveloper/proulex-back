...
* Modified by Javier Elías on 06/10/2022.
*/
...
FROM OrdenesVentaCancelaciones
	 ...
	 LEFT JOIN InventariosMovimientos AS mvtoCancelacion ON OVCD_OrdenVentaCancelacionDetalleId = mvtoCancelacion.IM_ReferenciaMovtoId AND ISNULL(hijos.OVD_ART_ArticuloId, padre.OVD_ART_ArticuloId) = mvtoCancelacion.IM_ART_ArticuloId AND mvtoCancelacion.IM_CMM_TipoMovimientoId = 2000439 -- Cancelación de nota de venta
	 INNER JOIN InventariosMovimientos AS mvtoEntrada ON ISNULL(hijos.OVD_OrdenVentaDetalleId, padre.OVD_OrdenVentaDetalleId) = mvtoEntrada.IM_ReferenciaMovtoId AND ISNULL(hijos.OVD_ART_ArticuloId, padre.OVD_ART_ArticuloId) = mvtoEntrada.IM_ART_ArticuloId
WHERE ISNULL(OVCD_RegresoLibro, 0) = 1
      ...
      AND mvtoCancelacion.IM_InventarioMovimientoId IS NULL
GO