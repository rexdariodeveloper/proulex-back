ALTER TABLE [dbo].[CXPFacturas] ADD [CXPF_DiasCredito] [int] NULL
GO

UPDATE CXPFacturas SET CXPF_DiasCredito = CAST(CMM_Referencia AS int)
FROM CXPFacturas
INNER JOIN ControlesMaestrosMultiples ON CMM_ControlId = CXPF_CMM_TerminoPagoId
GO

ALTER TABLE [dbo].[CXPFacturas] ALTER COLUMN [CXPF_DiasCredito] [int] NOT NULL
GO

ALTER TABLE [dbo].[CXPFacturas] DROP CONSTRAINT [FK_CXPF_CMM_TerminoPagoId]
GO

ALTER TABLE [dbo].[CXPFacturas] DROP COLUMN [CXPF_CMM_TerminoPagoId]
GO

DELETE FROM [dbo].[ControlesMaestrosMultiples] WHERE [CMM_Control] = 'CMM_CXPF_TerminosPago'
GO
