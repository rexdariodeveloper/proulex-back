...
BEGIN
	...
	SELECT 
		...
		[PROGRU_Grupo] + COALESCE([dbo].[fn_getProyeccionUltimoGrupoCreado]([PROGRU_SUC_SucursalId], [PROGI_ProgramaIdiomaId], PAMOD_ModalidadId, [PROGRU_Nivel] + 1, inicio), 0) as maxgrupo,
		...
		[dbo].[fn_getProyeccionGrupoCodigo]([PROGRU_GrupoId],  ([PROGRU_Grupo] - 1) + COALESCE([dbo].[fn_getProyeccionUltimoGrupoCreado]([PROGRU_SUC_SucursalId], [PROGI_ProgramaIdiomaId], PAMOD_ModalidadId, [PROGRU_Nivel] + 1, inicio), 0),  PROGRU_Nivel) as codigo,
		[PROGRU_Aula],
		[PROGRU_GrupoId] as grupoReferenciaId,
		2000620 as estatusId,
		@creadoPorId as creadoPorId,
		CASE WHEN (PROGRU_Nivel + 1) > PROGI_NumeroNiveles THEN 'No se puede proyectar un grupo de nivel final' ELSE NULL END as error
