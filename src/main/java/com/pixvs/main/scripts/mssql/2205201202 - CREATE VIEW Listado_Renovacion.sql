 ...
 *****************************************************************
*/
CREATE OR ALTER VIEW [dbo].[VW_Listado_Renovacion]
AS
	WITH TipoContrato AS (
		...
		FROM SolicitudesRenovacionesContratacionesDetalles srcd
			INNER JOIN EmpleadosContratos ec ON srcd.SRCD_EMP_EmpleadoId = ec.EMPCO_EMP_EmpleadoId AND ec.EMPCO_CMM_EstatusId = 2000952
			INNER JOIN ControlesMaestrosMultiples cmm ON ec.EMPCO_CMM_TipoContratoId = cmm.CMM_ControlId
		WHERE srcd.SRCD_CMM_EstatusId = 2000950
	...
	SELECT
		...
		(SELECT COUNT(*) FROM SolicitudesRenovacionesContratacionesDetalles srcd WHERE srcd.SRCD_SRC_SolicitudRenovacionContratacionId = src.SRC_SolicitudRenovacionContratacionId AND srcd.SRCD_CMM_EstatusId = 2000950) as cantidadEmpleado,
	...
	WHERE src.SRC_CMM_EstatusId = 2000952
