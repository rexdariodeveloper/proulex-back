...
CREATE OR ALTER VIEW [dbo].[VW_REPORTE_HISTORIAL_COMPRA] AS
	select 
	...
	from OrdenesCompra
		...
		inner join OrdenesCompraRecibos on OCD_OrdenCompraDetalleId = OCR_OCD_OrdenCompraDetalleId
		inner join CXPFacturasDetalles on OCR_OrdenCompraReciboId = CXPFD_OCR_OrdenCompraReciboId
		inner join UnidadesMedidas cxpf_um on CXPFD_UM_UnidadMedidaId = cxpf_um.UM_UnidadMedidaId
		inner join CXPFacturas on CXPFD_CXPF_CXPFacturaId = CXPF_CXPFacturaId
		inner join ControlesMaestrosMultiples cmm_cxpf on CXPF_CMM_EstatusId = cmm_cxpf.CMM_ControlId
GO