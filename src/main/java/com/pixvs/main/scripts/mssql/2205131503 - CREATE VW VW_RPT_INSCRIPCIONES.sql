...
AS
     SELECT Sedes.SUC_SucursalId AS sedeInscripcionId,
			Sedes.SUC_Nombre AS sedeInscripcion,
			INS_InscripcionId AS inscripcionId,
			INS_Codigo AS inscripcion,
			ISNULL(INS_FechaModificacion, INS_FechaCreacion) AS fechaInscripcion,
			OV_OrdenVentaId AS notaVentaId,
			OV_Codigo AS notaVenta,
			/*estatusOV*/estatus.CMM_Valor AS estatusNotaVenta,
