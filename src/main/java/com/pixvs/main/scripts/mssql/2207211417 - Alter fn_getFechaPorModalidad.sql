...
AS BEGIN 
	...
	WHILE @counter <= @dias BEGIN
		...
		IF (Select CAST(CONCAT(YEAR(GETDATE()),'-',EMPDNLF_Mes,'-',EMPDNLF_Dia) AS DATE) as fecha from EmpresaDiasNoLaboralesFijos where CAST(CONCAT(YEAR(GETDATE()),'-',EMPDNLF_Mes,'-',EMPDNLF_Dia) AS DATE) = @ff AND EMPDNLF_Activo = 1) is null
			AND (SELECT EMPDNL_Fecha from EmpresaDiasNoLaborales WHERE EMPDNL_Fecha= @ff AND EMPDNL_Activo = 1) IS NULL
			BEGIN
			INSERT @fechasValidas
