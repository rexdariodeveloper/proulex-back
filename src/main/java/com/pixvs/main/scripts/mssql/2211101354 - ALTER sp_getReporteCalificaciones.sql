CREATE OR ALTER FUNCTION [dbo].[fn_getAlumnoPrimerCiclo]
(
	 @alumnoId INT 
)
RETURNS varchar(100)
...
BEGIN
	...
	RETURN (
		...
		inner join ProgramasGrupos p on i.INS_PROGRU_GrupoId = p.PROGRU_GrupoId
		inner join PACiclos pa
			on p.PROGRU_PACIC_CicloId = pa.PACIC_CicloId
	
		WHERE i.INS_ALU_AlumnoId = @alumnoId
		ORDER BY i.INS_FechaCreacion ASC
	)
