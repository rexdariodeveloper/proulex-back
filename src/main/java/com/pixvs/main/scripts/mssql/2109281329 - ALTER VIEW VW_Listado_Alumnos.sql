...
CREATE OR ALTER VIEW [dbo].[VW_Listado_Alumnos] AS
    ...
    SELECT
        ...
        CAST(DATEDIFF(MONTH,ALU_FechaNacimiento,GETDATE())/12 AS nvarchar(MAX)) + ' años' + CASE WHEN DATEDIFF(MONTH,ALU_FechaNacimiento,GETDATE())%12 = 0 THEN '' ELSE ', ' + CAST(DATEDIFF(MONTH,ALU_FechaNacimiento,GETDATE())%12 AS nvarchar(MAX)) + ' meses' END AS edad,
