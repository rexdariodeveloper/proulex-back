...
-- Modify date: 25/08/2022
-- Description:	la vista para obtener el reporte de calificaciones con detalle
-- Version 1.0.3
-- =============================================

CREATE OR ALTER VIEW [dbo].[VW_RPT_ReporteCalificacionesDetalles]
AS
	SELECT a.ALU_CodigoAlumnoUDG AS codigoAlumno,
		...
		dbo.fn_getAlumnoPrimerCiclo(ALU_AlumnoId) cohorte,
		N'Si' AS regular,
		N'Si' AS carta,
	...
	FROM AlumnosGrupos ag
		...
		left JOIN ProgramasGruposExamenesDetalles pied ON aec.AEC_PROGRUED_ProgramaGrupoExamenDetalleId = pied.PROGRUED_ProgramaGrupoExamenDetalleId
		left JOIN PAActividadesEvaluacion paae ON pied.PROGRUED_PAAE_ActividadEvaluacionId = paae.PAAE_ActividadEvaluacionId
