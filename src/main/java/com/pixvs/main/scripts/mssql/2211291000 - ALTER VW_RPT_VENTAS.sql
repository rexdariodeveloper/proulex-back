...
AS
	SELECT OVD_OrdenVentaDetalleId AS detalleId,
		   ...
		   USU_Nombre + ' ' + USU_PrimerApellido + ISNULL(' ' + USU_SegundoApellido, '') AS usuarioOV,
		   ...
		   ISNULL(CXCF_Serie + ' ', '') + CXCF_Folio AS folioFiscal,
	...
	FROM OrdenesVenta
		 ...
		 CROSS APPLY dbo.fn_getMontosDetalleOV(OVD_OrdenVentaDetalleId) AS montos
		 ...
		 (
				...
				) AS todo WHERE INSSG_CMM_EstatusId != 2000543	-- CMM_INSSG_Estatus Cancelada
		 ...
		 LEFT JOIN SucursalesCortesCajas ON OV_SCC_SucursalCorteCajaId = SCC_SucursalCorteCajaId
		 LEFT JOIN SucursalesPlanteles ON SCC_SP_SucursalPlantelId = SP_SucursalPlantelId
		 INNER JOIN Usuarios ON OV_USU_CreadoPorId = USU_UsuarioId
		 LEFT JOIN CXCFacturas ON OV_CXCF_FacturaId = CXCF_FacturaId AND CXCF_CMM_EstatusId != 2000493 -- No Cancelada
	...
	FROM OrdenesVenta
			...
			LEFT JOIN SucursalesCortesCajas ON OV_SCC_SucursalCorteCajaId = SCC_SucursalCorteCajaId
			LEFT JOIN SucursalesPlanteles ON SCC_SP_SucursalPlantelId = SP_SucursalPlantelId
			INNER JOIN Usuarios ON OVC_USU_CreadoPorId = USU_UsuarioId
			LEFT JOIN CXCFacturas ON OV_CXCF_FacturaId = CXCF_FacturaId AND CXCF_CMM_EstatusId != 2000493 -- No Cancelada
	WHERE LEN(OV_Codigo) != 36
		AND OV_MPPV_MedioPagoPVId IS NOT NULL
GO