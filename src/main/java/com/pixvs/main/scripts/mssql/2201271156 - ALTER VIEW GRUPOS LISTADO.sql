...
AS
	Select 
		...
		CASE
		WHEN DATEADD(DAY,COALESCE((SELECT CAST(CMA_Valor as INT) FROM ControlesMaestros WHERE CMA_Nombre='CM_SUMA_DIAS_FECHA_FIN'),0),cast(PROGRU_FechaFin as DATE)) < cast (GETDATE() as DATE) and PROGRU_Activo = 1 AND datepart(hour, PAMODH_HoraInicio) < datepart(hour, getdate()) THEN 'Finalizado'
		WHEN DATEADD(DAY,COALESCE((SELECT CAST(CMA_Valor as INT) FROM ControlesMaestros WHERE CMA_Nombre='CM_SUMA_DIAS_FECHA_FIN'),0),cast(PROGRU_FechaFin as DATE)) >= cast (GETDATE() as DATE) and PROGRU_Activo = 1 AND datepart(hour, PAMODH_HoraInicio) >= datepart(hour, getdate()) THEN 'Activo'
