...
CREATE OR ALTER VIEW [dbo].[VW_InscripcionesPendientesPCP_Alumnos] WITH SCHEMABINDING AS
    ...
    SELECT
        ...
        CONCAT(ALU_Codigo,'|',ALU_Nombre,' ',ALU_PrimerApellido,' ' + ALU_SegundoApellido,' ',ALU_Nombre,'|',CONCAT(PROG_Codigo,' ',idioma.CMM_Valor),'|',COALESCE(ALU_Dependencia,''),'|',PROGRU_Codigo) AS textoBusqueda
    FROM [dbo].[Alumnos]
