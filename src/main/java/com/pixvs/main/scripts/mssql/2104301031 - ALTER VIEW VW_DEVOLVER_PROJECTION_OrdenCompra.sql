...
ALTER VIEW [dbo].[VW_DEVOLVER_PROJECTION_OrdenCompra] AS
...
SELECT
	...
	}',',') + ']' AS NVARCHAR(MAX)),'[]') AS detallesStr
FROM(
	SELECT
		id,
		codigo,
		...
		COALESCE(CAST('[' + STRING_AGG('{
			...
			"cantidadPendienteDevolver": ' + CAST((OCR_CantidadRecibo + totalDevolver) AS varchar) + ',
			"devoluciones": ' + devoluciones + ',
			"cxpFacturasDetalles": ' + cxpFacturasDetalles + ',
			"ordenCompra": null,
			"evidencia": ' +  evidencia + '
		}',',') + ']' AS NVARCHAR(MAX)),'[]') AS recibosPendientes
	FROM(
		SELECT
			id,
			codigo,
	...
	WHERE CXPFD_CXPFacturadetalleId IS NULL
