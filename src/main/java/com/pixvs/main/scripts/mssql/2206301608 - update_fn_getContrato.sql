...
ALTER   FUNCTION [dbo].[fn_getContrato] (
    @idEmpleado INTEGER, @fechaInicio DATE, @fechaFin DATE, @idModalidad INTEGER, @idCiclo INTEGER, @idCmmIdioma INTEGER, @idPlantel INTEGER, @idIdioma INTEGER, @idSucursal INTEGER, @idCmmTipoContrato INTEGER
    )
RETURNS TABLE
AS
RETURN
(
    SELECT
        ...
        (
            SELECT UPPER(CONCAT(USU_Nombre,' ',USU_PrimerApellido,' ',USU_SegundoApellido)) FROM Usuarios
            INNER JOIN Sucursales on SUC_USU_ResponsableId = USU_UsuarioId WHERE SUC_SucursalId = PROGRU_SUC_SucursalId
        ) as directorSede,
    ...
    INNER JOIN VW_ProgramasGruposProfesores ON grupoId = PROGRU_GrupoId AND activo = 1
    ...
    AND (CASE WHEN PROGRU_CMM_TipoGrupoId = 2000392 THEN 2000391 ELSE PROGRU_CMM_TipoGrupoId END) = @idCmmTipoContrato
