/**
* Created by Angel Daniel Hernández Silva on 16/12/2021.
...
CREATE OR ALTER FUNCTION [dbo].[fn_ReinscripcionProjection_Alumnos](@sucursalId int, @textoBuscar varchar(255), @aprobados bit) RETURNS TABLE WITH SCHEMABINDING AS RETURN(
	...
	FROM (
		...
		FROM (
			...
			GROUP BY ALU_AlumnoId, PROGI_CMM_Idioma, PROG_ProgramaId, PROGI_ProgramaIdiomaId, PROGI_NumeroNiveles, ALUG_CalificacionFinal, ALUG_CMM_EstatusId
	...
	WHERE
		...
		AND CONCAT(COALESCE(BECU_CodigoBeca,''),'|',CONCAT(PROG_Codigo,' ',CMM_Valor),'|',PAMOD_Nombre,'|',PAMODH_Horario) LIKE @textoBuscar
)
GO