...
(
	...
	SELECT
		...
		montoTotal AS total,
		'' totalFiscal,
		(CASE WHEN INS_CMM_EstatusId = 2000512 THEN 'CURSO GUARDADO' ELSE COALESCE(PROGRU_Codigo, 'CURSO GUARDADO') END ) AS claveCurso,
	...
	INNER JOIN (
		...
		GROUP BY id, codigo, fecha, fechaPago, estatusId, sucursalCorteCajaId, medioPagoPVId, referenciaPago, montoDescuento, montoTotal, detallePadreId, detallePadreArticuloId, detallePadreMontoCurso, detallePadreMontoOtros, folioFactura
