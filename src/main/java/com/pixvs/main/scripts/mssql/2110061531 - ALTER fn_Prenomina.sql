...
AS BEGIN
	...
	FROM(
		...
		FROM(
			...
			WHERE PRENO_PrenominaId is null AND ((PROGRU_FechaFin > @fechaInicioQuincena  AND PROGRU_FechaFin < @fechaFinQuincena) OR (PROGRU_FechaInicio > @fechaInicioQuincena  AND PROGRU_FechaInicio < @fechaFinQuincena) ) AND PROGRU_Activo = 1
