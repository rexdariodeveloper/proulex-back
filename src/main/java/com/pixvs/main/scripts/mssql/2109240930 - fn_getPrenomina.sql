	...
	FROM(
		...
		FROM(
			...
			select
				...
				0 as deduccion,
				(Select count(*) from dbo.fn_getDiaHabiles(@fechaInicioQuincena,@fechaFinQuincena,PAMOD_Domingo,PAMOD_Lunes,PAMOD_Martes,PAMOD_Miercoles,PAMOD_Jueves,PAMOD_Viernes,PAMOD_Sabado))* DATEDIFF(HOUR,PAMODH_HoraInicio,PAMODH_HoraFin) as horasTotales,
		...
		FROM(
			...
			left join Programas on PROG_ProgramaId = PROGI_PROG_ProgramaId
			left join ControlesMaestrosMultiples CMM on CMM_ControlId = PROGI_CMM_Idioma
			INNER JOIN Prenominas on PRENO_PROGRU_GrupoId = PROGRU_GrupoId
			WHERE PRENO_FechaFinQuincena >= PROGRULC_Fecha AND PRENO_FechaInicioQuincena <= PROGRULC_Fecha
		) Q2
		...
		FROM(
			...
			INNER JOIN Prenominas on PRENO_EDP_EmpleadoDeduccionPercepcionId = EDP_EmpleadoDeduccionPercepcionId
			WHERE PRENO_FechaFinQuincena >= EDP_Fecha AND PRENO_FechaInicioQuincena <= EDP_Fecha
		) Q3
		...
		where fecha is not null AND concat(PROGRU_GrupoId,fecha) not in (Select concat(PRENO_PROGRU_GrupoId,PRENO_DiasFestivo) from Prenominas where PRENO_DiasFestivo >= @fechaInicioQuincena and PRENO_DiasFestivo <=@fechaFinQuincena)
		)Q5
