/******
    Object:  View [dbo].[VW_RPT_CXPSolicitudesPagosDetalles]
    Script Date: 25/10/2021
    Add: totalAutorizado
******/
SET ANSI_NULLS ON
GO
...
ALTER   VIEW [dbo].[VW_RPT_CXPSolicitudesPagosDetalles] AS
SELECT
...
LEFT JOIN (
	...
	SELECT CXPSD_CXPS_CXPSolicitudPagoId, COUNT ( DISTINCT cxpf.idProveedor ) as proveedores,
		...
		sum(CXPSD_MontoProgramado) as total, sum(CXPSD_MontoProgramado) as totalMN,
		sum(CASE WHEN (CXPSD_CMM_EstatusId = 2000163 OR CXPSD_CMM_EstatusId = 2000164 OR CXPSD_CMM_EstatusId = 2000166) THEN 0 ELSE CXPSD_MontoProgramado END) as totalAutorizado,
		sum(cxpf.montoPagado) as totalPagado, sum(cxpf.montoPagadoMN) as totalPagadoMN,
		sum(cxpf.total) as totalFactura
...
LEFT JOIN (
	...
	SELECT cxpf.codigoProveedor, cxpf.nombreProveedor, cxpf.proveedorRFC, CXPSD_CXPS_CXPSolicitudPagoId, cmm_es.CMM_Valor as detalleEstatus, cmm_es.CMM_ControlId as detalleEstatusId,
		...
		COALESCE(oc.folio, srv.folio,PagosRh.codigoRh, '-') as folio, cxpf.total as totalServicio,CASE WHEN ( (CXPSD_MontoProgramado + cxpf.montoPagado) = cxpf.total) THEN 0 ELSE (cxpf.total - CXPSD_MontoProgramado) END as saldo,
		beneficiario
	FROM CXPSolicitudesPagosDetalles
