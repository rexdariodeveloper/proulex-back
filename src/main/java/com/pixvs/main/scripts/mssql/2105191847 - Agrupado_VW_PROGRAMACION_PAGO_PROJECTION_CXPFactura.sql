...
ALTER   VIEW [dbo].[VW_PROGRAMACION_PAGO_PROJECTION_CXPFactura] AS



SELECT
	id, MAX(proveedorId) proveedorId, MAX(codigoRegistro) codigoRegistro, MAX(montoRegistro) montoRegistro,
	MAX(saldo) saldo, MAX(montoProgramado) montoProgramado, MAX(fechaRegistro) fechaRegistro,
	STRING_AGG(ordenCompraTexto, ', ') ordenCompraTexto,
	MAX(folioSolicitudPagoServicio) folioSolicitudPagoServicio, MAX(evidenciaStr) evidenciaStr,
	MAX(evidenciaRh) evidenciaRh, MAX(facturaPDFStr) facturaPDFStr, MAX(facturaXMLStr) facturaXMLStr,
	MAX(cxpSolicitudesPagosServiciosStr) cxpSolicitudesPagosServiciosStr, MAX(diasCredito) diasCredito, MAX(fechaModificacion) fechaModificacion,
	MAX(codigoMoneda) codigoMoneda,
	STRING_AGG(sucursal, ', ') sucursal,
	MAX(beneficiario) beneficiario
FROM(
SELECT
...
LEFT JOIN (
	...
	WHERE CXPSD_CMM_EstatusId IN (2000161,2000165) AND ALE_CMM_EstatusId NOT IN(1000152, 1000155, 1000153)
	GROUP BY CXPF_CXPFacturaId
