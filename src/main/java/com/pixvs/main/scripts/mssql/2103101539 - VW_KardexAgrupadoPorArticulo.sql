CREATE OR ALTER VIEW [dbo].[VW_KardexAgrupadoPorArticulo] AS
	...
	from (
		...
		SELECT IM_InventarioMovimientoId as id,  IM_FechaCreacion as fechaMovimiento, IM_ART_ArticuloId as idArticulo, IM_LOC_LocalidadId as idLocalidad,
				COALESCE( SUM(IM_Cantidad) OVER (PARTITION BY  IM_ART_ArticuloId 
									ORDER BY IM_ART_ArticuloId, IM_FechaCreacion, IM_InventarioMovimientoId
...
--ORDER BY idArticulo, fechaMovimiento, IM_InventarioMovimientoId