...
(
	...
	FROM
	(
		SELECT SCC_SucursalCorteCajaId AS corteId,
			   SUC_Nombre AS sede,
			   ...
			   montoTotal AS total,
			   '' totalFiscal,
			   (CASE WHEN esCurso = 0 OR (esCurso = 1 AND OVCD_OVD_OrdenVentaDetalleId IS NOT NULL) THEN '' WHEN(INS_CMM_EstatusId = 2000512 AND esCurso = 1) OR (esCurso = 1 AND PROGRU_Codigo IS NULL) THEN 'CURSO GUARDADO' ELSE COALESCE(PROGRU_Codigo, 'CURSO GUARDADO') END) AS claveCurso,
			 ...
			 (
				...
				(
					SELECT id,
						   ...
						   CASE WHEN ArticuloPrimario.ART_PROGI_ProgramaIdiomaId IS NOT NULL THEN impuestosPadre.Total ELSE 0 END AS detallePadreMontoCurso,
						   CASE WHEN ArticuloSecundario.ART_ARTST_ArticuloSubtipoId = 2 THEN impuestosHijo.Total ELSE 0 END AS montoLibro,
						   CASE WHEN ArticuloSecundario.ART_ARTST_ArticuloSubtipoId = 5 THEN impuestosHijo.Total ELSE 0 END AS montoExamen,
						   CASE WHEN ArticuloPrimario.ART_PROGI_ProgramaIdiomaId IS NULL THEN impuestosPadre.Total ELSE 0 END AS detallePadreMontoOtros,
						   ISNULL(CXCF.CXCF_Serie + ' ', '') + CXCF.CXCF_Folio AS folioFactura,
						   ...
						   impuestosPadre.Descuento AS descuentoDetalle,
						   impuestosPadre.Total AS totalDetalle,
					...
					FROM VW_OrdenesVenta
						 ...
						 CROSS APPLY [dbo].[fn_getImpuestosArticulo](DetallePadre.OVD_Cantidad,DetallePadre.OVD_Precio,DetallePadre.OVD_Descuento,CASE WHEN DetallePadre.OVD_IVAExento = 0 THEN DetallePadre.OVD_IVA ELSE 0 END,DetallePadre.OVD_IEPS,DetallePadre.OVD_IEPSCuotaFija) AS impuestosPadre
						 CROSS APPLY [dbo].[fn_getImpuestosArticulo](DetalleHijo.OVD_Cantidad,DetalleHijo.OVD_Precio,DetalleHijo.OVD_Descuento,CASE WHEN DetalleHijo.OVD_IVAExento = 0 THEN DetalleHijo.OVD_IVA ELSE 0 END,DetalleHijo.OVD_IEPS,DetalleHijo.OVD_IEPSCuotaFija)  AS impuestosHijo
						 LEFT JOIN CXCFacturas AS CXCF ON facturaId = CXCF.CXCF_FacturaId AND CXCF.CXCF_CMM_EstatusId NOT IN(2000490, 2000493) -- Borrada, Cancelada
				) AS OVDDesglosado
				GROUP BY id,
						 codigo,
						 fecha,
						 ...
						 totalDetalle,
						 grupoOriginalId
			 ) AS OVD ON sucursalCorteCajaId = SCC_SucursalCorteCajaId
	...
	SELECT SCC_SucursalCorteCajaId AS corteId,
			...
			1 AS esCancelacion,
			0 AS ovCancelada,
			0 AS descuentoDetalle,
			(OVC_ImporteReembolsar * -1) AS totalDetalle,
