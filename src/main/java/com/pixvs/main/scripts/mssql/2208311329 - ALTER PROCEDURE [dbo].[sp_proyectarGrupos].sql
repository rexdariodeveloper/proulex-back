...
BEGIN
	...
	SELECT 
		...
		[dbo].[fn_getProyeccionGrupoFechaFin]([fin],    [PAMOD_DiasFinPeriodoInscripcionBeca], [PAMOD_ModalidadId]) as fechaFinInscripcionesBecas,
	...
	INNER JOIN(
		SELECT 
		[PACD_PAMOD_ModalidadId], [PACD_CMM_IdiomaId], [PACD_FechaInicio], [PACD_FechaFin],
		LEAD([PACD_FechaInicio]) OVER (ORDER BY [PACD_PAMOD_ModalidadId], [PACD_CMM_IdiomaId], [PACD_FechaInicio]) inicio,
		LEAD([PACD_FechaFin]) OVER (ORDER BY [PACD_PAMOD_ModalidadId], [PACD_CMM_IdiomaId], [PACD_FechaInicio]) fin
		FROM ProgramacionAcademicaComercialDetalles
	) f ON [PROGRU_PAMOD_ModalidadId] = f.[PACD_PAMOD_ModalidadId] AND [PROGI_CMM_Idioma] = f.[PACD_CMM_IdiomaId] AND [PROGRU_FechaInicio] = [PACD_FechaInicio]
	INNER JOIN [dbo].[PAModalidades] on [PROGRU_PAMOD_ModalidadId] = [PAMOD_ModalidadId]
	LEFT JOIN (
		SELECT [PROGRU_GrupoReferenciaId] referenciaId FROM [dbo].[ProgramasGrupos] WHERE [PROGRU_GrupoReferenciaId] IS NOT NULL
	...
	SELECT CONVERT(varchar(50),NEWID()) as codigoOV,OV_SUC_SucursalId as sucursalId,GETDATE() as fechaOV,GETDATE() as fechaRequerida,@monedaId as monedaId,0 as diasCredito,@medioPago as medioPago,@notaVentaEstatusId as estatusOvId,GETDATE() as fechaCreacion ,@creadoPorId as creadoporId,@metodoPago as metodoPago,
	...
	LEFT JOIN (
		SELECT 
			[INSSG_PROG_ProgramaId] programaId,
			[INSSG_PAMOD_ModalidadId] modalidadId,
			[INSSG_Nivel] nivel
		FROM [dbo].[InscripcionesSinGrupo]
		WHERE
			[INSSG_CMM_EstatusId] IN (2000540, 2000541)
	) inssg ON inssg.programaId = [PROGI_PROG_ProgramaId] AND inssg.modalidadId = [PROGRU_PAMOD_ModalidadId] AND inssg.nivel = [PROGRU_Nivel]
	WHERE 
		[INS_CMM_EstatusId] NOT IN(2000512 /*CANCELADAS*/) 
		AND inssg.programaId IS NULL /*Sin inscripción sin grupo*/
		AND [INS_PROGRU_GrupoId] IN ( SELECT VALUE FROM STRING_SPLIT(@gruposIds, ',')  ) OPTION(RECOMPILE);
	...
	SELECT OV_OrdenVentaId, ART_ArticuloId, unidadMedidaInventarioId, factorConversionVentas ,1.00, [dbo].[getPrecioVenta](padreId, ART_ArticuloId, lp.listadoPrecioId), 
