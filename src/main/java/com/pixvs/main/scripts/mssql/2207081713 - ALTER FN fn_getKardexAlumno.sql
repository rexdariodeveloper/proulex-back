...
ALTER FUNCTION [dbo].[fn_getKardexAlumno](@alumnoId int)RETURNS @tbl TABLE(
	...
	SELECT
		...
		(CASE WHEN LEN(COALESCE(OV_Codigo, '')) != 36 THEN COALESCE(OV_Codigo, '') ELSE '' END) AS alumnoOrdenVenta,
		CONVERT(BIT, CASE WHEN OV_CMM_EstatusId IN (2000507, 2000508) THEN 1 ELSE 0 END) AS imprimirOV, -- OV Facturada, Pagada
