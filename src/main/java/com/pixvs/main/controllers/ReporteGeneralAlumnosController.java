...
@RestController
@RequestMapping("/api/v1/reporte-general-alumnos")
public class ReporteGeneralAlumnosController {
    ...
    public JsonResponse getDatosByFiltros(@RequestBody JSONObject json) throws SQLException {
        ...
        return new JsonResponse(programaGrupoDao.getReporteGeneralAlumnos(sedeId, plantelesId, fechas, modalidadesId), null, JsonResponse.STATUS_OK);
    }
    ...
    public void templateDownload(@RequestBody JSONObject json, HttpServletResponse response, ServletRequest req) throws IOException, ParseException {
        ...
        String query = "SELECT COUNT(alumnoId) AS totalAlumnos,\n" +
                "       plantelAlumno,\n" +
                "       nivel\n" +
        ...
        String[] allColumnas = new String[]{"TOTAL DE ALUMNOS", "PREPARATORIA", "NIVEL"};
        ...
        query = "SELECT COUNT(alumnoId) AS totalAlumnos,\n" +
                "       plantelAlumno\n" +
                "FROM VW_RPT_ReporteGeneralGruposAlumnos\n" +
                "WHERE sedeAlumnoId = :sedesId\n" +
                "    AND (COALESCE(:plantelesId, 0) = 0 OR plantelAlumnoId IN (:plantelesId)) \n" +
                ...
                "    AND (COALESCE(:modalidadesId, 0) = 0 OR modalidadId IN (:modalidadesId))\n" +
                "GROUP BY plantelAlumno\n" +
                "ORDER BY plantelAlumno\n" +
                "OPTION(RECOMPILE)";

        allColumnas = new String[]{"TOTAL DE ALUMNOS", "PREPARATORIA"};

        ExcelExportadoHoja hojaHorario = new ExcelExportadoHoja();
        hojaHorario.setNombreHoja("Total por Preparatoria");
        ...
        allColumnas = new String[]{"TOTAL DE ALUMNOS", "NIVEL"};
        ...
        excelController.downloadXlsxMultipleSheet(response, "Reporte_General_Alumnos", allHojas, false);
    }
}